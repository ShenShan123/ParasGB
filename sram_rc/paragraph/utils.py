...
def get_balanced_edges(
    ...
    for i in range(g.tar_edge_dist.size(0)):
        ...
        # for neg edges - 处理空负边的情况
        if neg_edge_type.numel() == 0:
            # 如果没有负边，创建空张量
            logging.info(f"Edge type {i}, no negative edges available, skipping")
            neg_edge_index_list.append(torch.empty((2, 0), dtype=torch.long))
            neg_edge_type_list.append(torch.empty(0, dtype=neg_edge_type.dtype))
        else:
