...
class SealSramDataset(InMemoryDataset):
    def __init__(
        self,
        root, 
        train_cases,  # e.g., "1+5+7" -> case1_RC.pt, case5_RC.pt, case7_RC.pt
        test_cases,   # e.g., "15+17" -> case15_RC.pt, case17_RC.pt
        neg_edge_ratio=1.0,
        to_undirected=True,
        sample_rates=[1.0], 
        task_level='node',
        ...
        Args:
            root (str): The root directory of the dataset (e.g., './analog_rc/').
            train_cases (str): Case IDs for training, separated by '+'.
            test_cases (str): Case IDs for testing, separated by '+'.
            ...
            task_level (str): The task type. It can be 'node' or 'edge'.
            net_only (bool): Whether to only use net nodes.
        ...
        # Parse train and test dataset names (direct filenames like sandwich, ultra8t)
        ...
        # Track which indices are train vs test
        self.num_train = len(self.train_names)
        self.num_test = len(self.test_names)
        
        print(f"SealSramDataset: Train cases: {self.train_names}")
        print(f"SealSramDataset: Test cases: {self.test_names}")

        self.sample_rates = sample_rates
        # Extend sample_rates if needed
        if len(self.sample_rates) < len(self.names):
            self.sample_rates = self.sample_rates + [self.sample_rates[-1]] * (len(self.names) - len(self.sample_rates))
        
        self.folder = root  # 直接使用root作为数据目录 (e.g., '../sram/')
    ...
    def norm_nfeat(self, ntypes):
        for ntype in ntypes:
            ...
            print(f"normalizing node_attr {ntype} (数量: {num_nodes_of_type})")
        ...
        if self.task_level == 'edge':
            if self.data_type == 'r':
                ...
                edge_max = self._data.edge_label.max()
                if edge_max > 0:
                    self._data.edge_label = self._data.edge_label / edge_max
            ...
            y_max = self._data.y.max()
            if y_max > 0:
                self._data.y = self._data.y / y_max

            artificial_mask = self._data.y < 1e-4  # max归一化后用绝对值阈值过滤人工节点
    ...
    def sram_graph_load(self, name, raw_path):
        ...
        # Default power net ids for case files
        ...
        print(f"hg.node_types:{hg.node_types}")
        # assert 0 
        if 'device' in hg.node_types and hasattr(hg['device'], 'x'):
        ...
        """
        g._e2type = {}
        # print(f"enumerate(hg.edge_items()):{enumerate(hg.edge_items())}")
        # assert 0
        ...
        ## convert to undirected edgesion(g, name, self.class_boundaries)
    ...
    def raw_dir(self) -> str:
        # Raw files are in the folder (e.g., './sram_rc/sram/')
        return self.folder
    ...
    def processed_dir(self) -> str:
        ...
        base_name = 'processed_rcg' + suffix
        
        if self.task_level == 'edge':
            return os.path.join(self.root, base_name, 'edge')
        elif self.task_level == 'node':
            return os.path.join(self.root, base_name, 'node')
        else:
...
def performat_SramDataset(dataset_dir, 
                          train_cases,
                          test_cases,
    ...
    # Parse dataset names to determine sample rates
    train_names = [c.strip() for c in train_cases.split('+') if c.strip()]
    test_names = [c.strip() for c in test_cases.split('+') if c.strip()]
    ...
    for name in all_names:
        if name in large_datasets:
            sr_list.append(large_dataset_sample_rates)
        elif name in small_datasets:
            sr_list.append(small_dataset_sample_rates)
        else:
            print(f"警告: 未知数据集 {name}, 使用小数据集采样率")
            sr_list.append(small_dataset_sample_rates)
    
    print(f"数据集采样率: {dict(zip(all_names, sr_list))}")
    ...
    print(f"Building dataset train={train_cases}, test={test_cases} took {timestr}")
