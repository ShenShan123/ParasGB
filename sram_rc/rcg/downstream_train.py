...
def compute_loss(args, pred, true, criterion, sample_per_class=None):
    ...
    Args:
        ...
        sample_per_class: 每个类别的样本数量，用于 BalancedSoftmax
    ...
    if args.task == 'classification':
        # 使用 BalancedSoftmax 处理类别不平衡
        if sample_per_class is not None:
            loss = balanced_softmax_loss(pred, true, sample_per_class, reduction='mean')
        else:
...
def eval_epoch(args, loader, model, device, 
    ...
    for i, batch in enumerate(tqdm(loader, desc="eval_"+split, leave=False)):
        ...
        if args.task == 'regression':
            ...
            loss, predict_class, true = compute_loss(args, pred, class_true, criterion=criterion, sample_per_class=sample_per_class)
            _true = true.detach().to('cpu', non_blocking=True)
...
def regress_train(args, regressor, optimizer, criterion,
    ...
    log_file = os.path.join(log_dir, f"regression_{args.task_level}_{time.strftime('%Y%m%d_%H%M%S')}.txt")
    ...
    with open(log_file, 'w') as f:
        f.write(f"RCG 回归任务训练日志\n")
        f.write(f"训练数据集: {args.train_dataset}\n")
        f.write(f"测试数据集: {args.test_dataset}\n")
        f.write(f"任务级别: {args.task_level}\n")
        ...
        for epoch in range(args.epochs):
            ...
            train_res = logger.write_epoch(split='train')
            f.write(f"Epoch {epoch+1}/{args.epochs} - 训练: ")
            for k, v in train_res.items():
                f.write(f"{k}={v:.6f} " if isinstance(v, float) else f"{k}={v} ")
            f.write("\n")
        ...
        f.write(f"\n训练完成!\n")
        f.write(f"最佳模型在第 {best_results['best_epoch']+1} 轮, 验证MSE = {best_results['best_val_mse']:.6f}\n")
    
    print(f"训练日志已保存到 {log_file}")

def class_train(args, classifier, optimizer_classifier, 
          train_loader, val_loader, test_loaders, max_label,
          device, sample_per_class=None, scheduler=None):
    """
    ...
    log_dir = f"./logs/{dataset_name}"
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f"classification_{args.task_level}_{time.strftime('%Y%m%d_%H%M%S')}.txt")
    ...
    # 设置 criterion 用于 eval_epoch
    criterion = torch.nn.CrossEntropyLoss()
    
    with open(log_file, 'w') as f:
        f.write(f"RCG 分类任务训练日志\n")
        f.write(f"训练数据集: {args.train_dataset}\n")
        f.write(f"测试数据集: {args.test_dataset}\n")
        f.write(f"任务级别: {args.task_level}\n")
        f.write(f"模型: {args.model}\n\n")
        ...
        for epoch in range(args.epochs):
            ...
            for i, batch in enumerate(tqdm(train_loader, desc=f'Epoch:{epoch}')):
                ...
                # 使用 BalancedSoftmax
                if sample_per_class is not None:
                    class_loss = balanced_softmax_loss(class_logits, true_class, sample_per_class, reduction='mean')
                else:
            ...
            val_class_res = eval_epoch(
                args, val_loader,
                classifier, device, split='val', criterion=criterion, sample_per_class=sample_per_class
            )
            f.write(f"Epoch {epoch+1}/{args.epochs} - 验证: ")
            for k, v in val_class_res.items():
                f.write(f"{k}={v:.6f} " if isinstance(v, float) else f"{k}={v} ")
            f.write("\n")
            ...
            if val_class_res['f1'] > best_f1:
                ...
                for test_name in test_loaders.keys():
                    ...
                    test_class_res = eval_epoch(
                        args, test_loaders[test_name], 
                        classifier, device, split='test', criterion=criterion, sample_per_class=sample_per_class
                    )
                    test_class_results[test_name] = test_class_res
                    f.write(f"测试集 {test_name}: ")
                    for k, v in test_class_res.items():
                        f.write(f"{k}={v:.6f} " if isinstance(v, float) else f"{k}={v} ")
                    f.write("\n")
        ...
        f.write(f"\n训练完成!\n")
        f.write(f"最佳模型在第 {best_epoch+1} 轮, 验证F1 = {best_f1:.6f}\n")
...
def downstream_train(args, dataset, device):
    ...
    if args.model == 'pna':
        ...
        max_deg = int(deg.max()) + 1
        deg = torch.bincount(deg, minlength=max_deg)
        args.pna_deg = deg
        print(f"PNA degree histogram computed, max degree: {max_deg-1}")
    ...
    if args.task == 'regression':
        ...
        if hasattr(dataset, '_data') and hasattr(dataset._data, 'edge_label'):
            edge_label = dataset._data.edge_label
            if args.task_level == 'edge':
                # 边分类：edge_label 格式为 [regression_label, classification_label]
                if edge_label.dim() > 1 and edge_label.size(1) == 2:
                    class_labels = edge_label[:, 1].long()
                else:
                    class_labels = edge_label.long()
            else:
                # 节点分类
                if hasattr(dataset._data, 'y'):
                    class_labels = dataset._data.y.long()
                else:
                    class_labels = edge_label.long()
        ...
        class_train(args, model, optimizer, train_loader, val_loader, test_loaders, max_label,
                    device, sample_per_class=sample_per_class, scheduler=scheduler)
