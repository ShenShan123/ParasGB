...
def plot_distribution(labels, save_path, color=BAR_COLOR, xlabel='value', 
                      xlim=None, use_log_scale=False, add_lines=True):
    """绘制单个分布图（统一格式）
    ...
    if add_lines:
        _add_mean_median_lines(ax, labels)
...
def normalize_coupling_capacitance(edge_y, return_raw=False):
    """归一化耦合电容（cc_p2n, cc_p2p, cc_n2n边）- max归一化（无log变换，对比实验）"""
    ...
    # max 归一化（无log变换，对比实验）
    edge_max = filtered.max()
    if edge_max > 0:
        normalized = filtered / edge_max
    else:
        normalized = filtered
...
def normalize_ground_capacitance(node_y, return_raw=False):
    ...
    # normalized = torch.clamp(normalized, 0.0, 1.0)
    node_max = filtered.max()
    if node_max > 0:
        normalized = filtered / node_max
    else:
        normalized = filtered        
