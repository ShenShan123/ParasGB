...
def downstream_train(args, dataset, device, cl_embeds=None):
    ...
    Args:
        ...
        device (torch.device): The device to train the model on
        cl_embeds (torch.tensor): The node embeddings come from the contrastive learning
    """
    train_dataset = dataset['train']
    test_dataset = dataset['test']
    
    if args.sgrl:
        train_dataset.set_cl_embeds(cl_embeds)
    ...
    if args.task == 'regression':
        ...
        if args.regress_loss == 'gai':
            gmm_path = train_gmm(train_dataset)
            ...
            _, bin_edges, bin_count = get_lds_weights(
                train_dataset._data.edge_label[:, 1], 
            ...
            train_dataset._data.edge_label[:, 1] = weights
