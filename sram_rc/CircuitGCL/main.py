...
if __name__ == "__main__":
    ...
    parser.add_argument("--small_dataset_sample_rates", type=float, default=1, help="The sample rate for small dataset.")
    parser.add_argument("--large_dataset_sample_rates", type=float, default=1, 
    ...
    # SGRL arguments
    parser.add_argument('--sgrl', type=int, default=1, help='Enable contrastive learning, i.e., SGRL.')
    ...
    parser.add_argument('--cl_epochs', type=int, default=200, help='cl_epochs')
    ...
    parser.add_argument('--cl_batch_size', type=int, default=512, 
    ...
    parser.add_argument("--hid_dim", type=int, default=96, help="Hidden layer dim.")
    ...
    parser.add_argument('--num_classes',type=int,default=2,help='The number of classes for node classification.')
    ...
    if args.task == 'classification':
        log_filename = os.path.join(args.log_dir, f"{timestamp}_{args.task_level}_{args.task}_{args.train_dataset}_loss{args.class_loss}_batch{args.batch_size}.txt")
    else: # regression task
        log_filename = os.path.join(args.log_dir, f"{timestamp}_{args.task_level}_{args.task}_{args.train_dataset}_loss{args.regress_loss}_batch{args.batch_size}.txt")
    ...
    if args.sgrl == 1:
        ...
        embedding_path = os.path.join(embedding_path, f'embeddings_{args.train_dataset}_layer{args.num_gnn_layers}_dim{args.hid_dim}_{args.act_fn}.pkl')
        if os.path.exists(embedding_path):
            cl_embeds = torch.load(embedding_path)
        else:
            cl_embeds = sgrl_train(args, dataset['train'], device)
