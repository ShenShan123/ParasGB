...
class SealSramDataset(InMemoryDataset):
    ...
    def norm_nfeat(self, ntypes, num_classes=None):
        ...
        if self.task_level == 'edge':
            print(f"edge_label shape: {self._data.edge_label.shape}")
            ...
            print(f"node_label shape: {self._data.node_label.shape}")
    ...
    def single_g_process(self, idx: int):
        ...
        if self.task_level == 'node':
            ...
            # 归一化回归标签
            scale = 1e15
            reg_labels = torch.log1p(valid_labels * scale) / np.log1p(MAX_NODE_LABEL * scale)
            reg_labels = reg_labels.clamp(0, 1)
            ...
            # 归一化回归标签: log1p(y) / log1p(700)
            reg_labels = torch.log1p(pos_edge_y) / np.log1p(MAX_EDGE_LABEL)
            reg_labels = reg_labels.clamp(0, 1)
