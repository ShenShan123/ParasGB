...
def get_pos_neg_edges(
    ...
    r""" 为 RC 数据集生成负边
    RC 数据集只有一种目标边类型: pair_to (pin-pin)
    ...
    Return:
        neg_edge_index (LongTensor 2xN), neg_edge_type (LongTensor N)
    """
    ...
    if neg_ratio > 1.0 or sample_type == 'global':
        ...
        for i in range(neg_edge_index.size(1)):
            ...
            # RC 数据集只有 pair_to 边 (pin-pin)
            if ntypes == {PIN}:  # pin-pin
                neg_edge_type[i] = g.tar_edge_type[0] if g.tar_edge_type.numel() > 0 else 0
        
        # 只保留 pin-pin 类型的负边
        legal_mask = (g.node_type[neg_edge_index[0]] == PIN) & (g.node_type[neg_edge_index[1]] == PIN)
        print(
            f"Using global negtive sampling, #pos={g.tar_edge_index.size(1)}, " + 
            f"#neg={neg_edge_index[:, legal_mask].size(1)}")
        return neg_edge_index[:, legal_mask], neg_edge_type[legal_mask]
    ...
    for i in range(g.tar_edge_dist.size(0)):
        ...
        tar_edge_type_val = g.tar_edge_type[0].item() if g.tar_edge_type.numel() > 0 else 0
        neg_edge_type += [tar_edge_type_val] * indices.size(0)
