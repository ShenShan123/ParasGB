...
from sram_dataset import MAX_EDGE_LABEL, MAX_NODE_LABEL



NODE_LABEL_SCALE = 1e15


def _flatten_metric_input(values):
...
def denormalize_edge_label(values):
    values = np.clip(_flatten_metric_input(values), 0.0, 1.0)
    return np.power(10.0, values * 6.0) / 1e21


def denormalize_node_label(values):
    values = np.clip(_flatten_metric_input(values), 0.0, 1.0)
    return np.power(10.0, values * 6.0) / 1e21


def denormalize_regression(values, task_level):
    if task_level == 'edge':
        return denormalize_edge_label(values)
    if task_level == 'node':
        return denormalize_node_label(values)
    raise ValueError(f"Unsupported task level for denormalization: {task_level}")
...
class Logger (object):
    ...
    def __init__(self, task='classification', task_level='edge'):
        super().__init__()
    ...
    def write_epoch(self, split=""):
        ...
        if self.task == 'classification':
            ...
            true = _flatten_metric_input(true)
            pred_score = _flatten_metric_input(pred_score)
            res = {
                'loss': reformat(self._loss / self._size_current),
                'mae': reformat(mean_absolute_error(true, pred_score)),
                'mse': reformat(mean_squared_error(true, pred_score)),
                'rmse': reformat(_rmse(true, pred_score)),
                'r2': reformat(r2_score(true, pred_score)),
            }
            true_denorm = denormalize_regression(true, self.task_level)
            pred_denorm = denormalize_regression(pred_score, self.task_level)
            if self.task_level == 'edge':
                res.update({
                    'mae_ohm': reformat(mean_absolute_error(true_denorm, pred_denorm)),
                    'mse_ohm2': reformat(mean_squared_error(true_denorm, pred_denorm)),
                    'rmse_ohm': reformat(_rmse(true_denorm, pred_denorm)),
                })
            elif self.task_level == 'node':
                ff_scale = NODE_LABEL_SCALE
                res.update({
                    'mae_f': reformat(mean_absolute_error(true_denorm, pred_denorm)),
                    'mse_f2': reformat(mean_squared_error(true_denorm, pred_denorm)),
                    'rmse_f': reformat(_rmse(true_denorm, pred_denorm)),
                    'mae_ff': reformat(mean_absolute_error(true_denorm, pred_denorm) * ff_scale),
                    'mse_ff2': reformat(mean_squared_error(true_denorm, pred_denorm) * (ff_scale ** 2)),
                    'rmse_ff': reformat(_rmse(true_denorm, pred_denorm) * ff_scale),
                })
...
def eval_epoch(loader, model, device, 
               split='val', task='classification', task_level='edge'):
    """ 
    ...
    logger = Logger(task=task, task_level=task_level)
...
def train(args, model, optimizer, 
    ...
    with open(log_file, 'w') as f:
        ...
        for epoch in range(args.epochs):
            logger = Logger(task=args.task, task_level=args.task_level)
            model.train()
            ...
            # 验证
            val_logger = Logger(task=args.task, task_level=args.task_level)
            model.eval()
            with torch.no_grad():
            ...
            if is_best:
                ...
                for test_name, test_loader in test_loaders.items():
                    test_logger = Logger(task=args.task, task_level=args.task_level)
                    model.eval()
                    with torch.no_grad():
...
def train_node(args, model, optimizer, 
    ...
    with open(log_file, 'w') as f:
        ...
        for epoch in range(args.epochs):
            logger = Logger(task=base_task, task_level=args.task_level)
            model.train()
            ...
            # 验证
            val_results = eval_node_epoch(
                val_loader, model, device, 'Val', base_task, args.task_level
            )
            f.write(f"Epoch {epoch+1}/{args.epochs} - 验证: ")
            for k, v in val_results.items():
                f.write(f"{k}={v:.6f} ")
            ...
            if is_best:
                ...
                # 测试
                for test_name, test_loader in test_loaders.items():
                    test_results = eval_node_epoch(
                        test_loader, model, device,
                        f'Test_{test_name}', base_task, args.task_level
                    )
                    f.write(f"测试集 {test_name}: ")
                    for k, v in test_results.items():
                        f.write(f"{k}={v:.6f} ")
...
@torch.no_grad()
def eval_node_epoch(
    loader, model, device, split='val', task='regression', task_level='node'
):
    """
    评估节点任务模型
    Args:
