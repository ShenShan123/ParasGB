...
class SealSramDataset(InMemoryDataset):
    ...
    def norm_nfeat(self, ntypes):
        ...
        if self.is_node_task:
            # 归一化节点标签 (电容值): log1p(y * 1e15) / log1p(MAX * 1e15)
            scale = 1e15
            self._data.node_label = torch.log1p(self._data.node_label * scale) / np.log1p(MAX_NODE_LABEL * scale)
            self._data.node_label = self._data.node_label.clamp(0, 1)
            ...
            # 归一化边标签 (电阻值): log1p(y) / log1p(700)
            self._data.edge_label = torch.log1p(self._data.edge_label) / np.log1p(MAX_EDGE_LABEL)
            self._data.edge_label = self._data.edge_label.clamp(0, 1)
