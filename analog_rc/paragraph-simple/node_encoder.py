...
NODE_TYPE_ID_TO_NAME = {idx: name for name, idx in NODE_TYPE_NAME_TO_ID.items()}
...
class NodeFeatureEncoder(nn.Module):
    def __init__(self, out_dim, feat_config=None, default_vocab_size=100):
        ...
        self.feat_config = feat_config or DEFAULT_FEAT_CONFIG
    ...
    def forward(self, node_attr, node_type):
        ...
        for type_name, type_id in NODE_TYPE_NAME_TO_ID.items():
            ...
            for i, (idx, vocab_size) in enumerate(
                ...
                key = f"discrete_{i}"
                feat = type_attr[:, idx].round().long().clamp_(0, vocab_size - 1)
