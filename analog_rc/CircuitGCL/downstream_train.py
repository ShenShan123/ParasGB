...
class Logger (object):
    ...
    def write_epoch(self, split=""):
        ...
        if self.task == 'classification':
            ...
            f1 = f1_score(true[mask], pred_score[mask], average='macro', zero_division=0)
            precision = precision_score(true[mask], pred_score[mask], average='macro', zero_division=0)
            recall = recall_score(true[mask], pred_score[mask], average='macro', zero_division=0)
    ...
    for epoch in range(args.epochs):
        ...
        if best_results['best_val_mse'] > val_res['mse']:
            ...
            for test_name in test_loaders.keys():
                ...
                best_results['test_results'][test_name] = res
        ...
        # 打印每个测试集的结果
        for test_name, res in best_results['test_results'].items():
            print(f" Test [{test_name}]: {res}")
        print( "=====================================")
    ...
    for epoch in range(args.epochs):
        ...
        if val_class_res['f1'] > best_f1:
            ...
            for test_name in test_loaders.keys():
                ...
                best_test_results[test_name] = test_class_res
        
        print( "=====================================")
        print(f" Best epoch: {best_epoch}, best f1: {best_f1}")
        # 打印每个测试集的结果
        for test_name, res in best_test_results.items():
            print(f" Test [{test_name}]: {res}")
        print( "=====================================")
